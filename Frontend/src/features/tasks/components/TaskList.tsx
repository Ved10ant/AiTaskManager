    ...
    return (
        <div className="flex flex-col gap-4 mt-4">
            {visibleTasks.map((task: any) => (
                <div key={task._id} className="p-5 border w-500 transition-colors rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/5 border-white/10 hover:bg-white/10 w-full">
                    <div className="flex-1 w-full">
                        <div className="flex items-center gap-3 mb-2">
                            <h3 className={`font-bold text-lg ${user?.role === 'admin' ? 'text-white' : 'text-gray-400'}`}>{task.title}</h3>
                    ...
                    <div className="flex flex-col items-start md:items-end gap-2 text-sm min-w-[150px] w-full md:w-auto">
                        <span className="flex items-center gap-1 text-gray-400">
                            <Clock size={14} className={user?.role === 'admin' ? 'text-gray-400' : 'text-purple-400'} />
                        ...
                        {task.assignedTo ? (
                            <div className="mt-1 flex items-center gap-2 px-3 py-1 rounded-full border bg-white/10 border-white/10">
                                ...
                                <span className="font-medium text-xs truncate max-w-[100px] text-gray-300">{task.assignedTo.name}</span>
                                {user?.role === 'admin' && (
                                    <button
                                        onClick={() => { setSelectedMember(task.assignedTo); setSelectedTaskId(task._id); setShowProfile(true); }}
                                        className="p-1 rounded transition-colors text-gray-400 hover:bg-gray-400/20"
                                    >
                            ...
                            user?.role === 'employee' && (
                                <div className="flex flex-col items-end gap-2 w-full">
                                    <button
                                        ...
                                        className="text-xs flex items-center justify-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-2 rounded-lg transition-colors border border-indigo-500/50 shadow-lg shadow-indigo-500/20 font-bold"
                                    >
